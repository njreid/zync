...
- `object Crypto`:
  ...
  - `fun generateSelfSignedCert(cn: String = "zync"): ServerIdentity` — returns `ServerIdentity(keyStoreBytes: ByteArray, keyStorePassword: CharArray, keyAlias: String, certFingerprintSha256: String /* hex, colon-separated */)`. **PREFERRED implementation: Ktor's `buildKeyStore { certificate(alias){ password=…; domains=listOf("127.0.0.1","0.0.0.0") } }`** from `io.ktor:ktor-network-tls-certificates` (add to catalog) — returns a `KeyStore` directly, well-tested, pairs natively with `sslConnector`. Serialize it to PKCS12 bytes for persistence. Only hand-roll X.509 via BouncyCastle if buildKeyStore can't set a long validity or SAN entries you need. Fingerprint = SHA-256 over the cert's DER encoding.
...
- [ ] **Step 1: Use Netty (decision already made — controller confirmed via Ktor docs).** Ktor's official `sslConnector` examples all use **Netty**; CIO does not support server-side TLS in 3.5.1. Add `io.ktor:ktor-server-netty` to the catalog and switch the `embeddedServer(CIO, …)` in `ZyncServer` to `embeddedServer(Netty, …)`, using a custom-environment/`ApplicationEngine.Configuration` block: `connector { port=httpPort; host="127.0.0.1" }` for loopback plus `sslConnector(keyStore, keyAlias, { pw }, { pw }) { port=lan.tlsPort; host=lan.host }` when `lan != null`. Keep the engine choice in ONE place. The module (`zyncModule`) is engine-agnostic and must NOT change. Update `ServerAuthTest`/`ZyncServerSmokeTest`/`DevServer` only if the `embeddedServer(...)` construction signature they rely on changes — they call `ZyncServer.start()`, so if that API is stable they need no edits. Verify the loopback smoke + Playwright still pass on Netty. (Quick spike still welcome to confirm Netty binds cleanly under Robolectric — if Netty misbehaves in Robolectric's JVM, note it; the real bind is validated on the emulator in Task 8.)
